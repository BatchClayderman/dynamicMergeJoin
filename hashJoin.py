...
class HashJoin:
	...
	def join(self:object) -> int:
		...
		# Main Algorithm #
		print("Start to perform the \"hash join\" operation driven by the {0} database. ".format("smaller" if self.__drivenByTheSmallerOne else "larger"))
		startTime = time()
		...
		if self.__drivenByTheSmallerOne: # use the smaller one to drive the larger one
			...
			while n0: # self.__akasFilePointer
				if n0 in d:
					...
					del d[identifier] # remove the stored record to speed up
				else:
					n0, n1, n2 = self.__readAkasFile()
			...
			while m0: # self.__basicsFilePointer
				if m0 in d:
					...
					if not cnt % 1000000:
						timeDelta = time() - startTime
						print("Successfully write {0} datum lines to \"{1}\" in {2:.3f} second(s) at {3:.3f} datum/s. ".format(cnt, self.__outputFilePath, timeDelta, cnt / timeDelta))
					del d[m0] # remove the stored record to speed up
				m0, m1 = self.__readBasicsFile()
		...
		timeDelta = time() - startTime
		self.__outputFilePointer = None
		print("Finish performing the \"hash join\" operation with {0} datum lines written in {1:.3f} second(s) at {2:.3f} datum/s. ".format(cnt, timeDelta, cnt / timeDelta))
...
def main() -> int:
	hashJoin = HashJoin()
	if hashJoin.initialize():
		iRet = EXIT_SUCCESS if hashJoin.join() else EXIT_FAILURE
	else:
		iRet = EOF
