...
class DynamicMergeJoin:
	...
	def __closeInputFilePointer(self:object, idx:int) -> bool:
		if isinstance(idx, int) and -len(self.__inputFilePointers) <= idx < len(self.__inputFilePointers):
			if self.__inputFilePointers[idx] is None:
				self.__print("The input file \"{0}\" has already been closed. No need to close it again. ".format(self.__inputFilePaths[idx]), Debug)
				return True
			else:
				try:
					self.__inputFilePointers[idx].close()
					self.__inputFilePointers[idx] = None
					self.__print("Successfully close \"{0}\". ".format(self.__inputFilePaths[idx]), Info)
					return True
	...
	def initialize(								\
		...
		outputFilePath:str = "output.dynamicMergeJoin.tsv", 				\
	...
	def __readInputFile(self:object, idx:int) -> list:
		if isinstance(idx, int) and -len(self.__inputFilePointers) <= idx < len(self.__inputFilePointers) and self.__inputFilePointers[idx]:
			...
			exec(input(">>> "))
			self.__print("Nothing is read since the passed index \"{0}\" is invalid or the file has already been closed. ".format(idx), Error)
			self.__flag = 0
			return None
	def __getIdentifier(self:object, lines:list) -> tuple: # only the type list is allowed for the variables ``lines``
		identifier = max([line[0] for line in lines if line is not None])
		for idx in range(len(lines)):
			while lines[idx] is not None and lines[idx][0] != identifier:
				lines[idx] = self.__readInputFile(idx)
		return identifier
	def join(self:object) -> bool:
		# Status Checking #
		if self.__flag > 1:
			self.__flag = 1
		...
		# Main Algorithm #
		self.__print("Start to process the dynamic \"merge join\" operation. Please use \"Ctrl + C\" to stop it if it is necessary. ", Info)
		startTime = time()
		...
		try:
			while self.__flag:
				# Identifier Getting #
				identifier = self.__getIdentifier(lines)
			...
			self.__print("The dynamic \"merge join\" operation is interrupted by users. ", Warning)
			self.__print("Successfully write {0} datum lines to \"{1}\" in {2:.3f} second(s) at {3:.3f} datum/s. ".format(lineCount, self.__outputFilePath, timeDelta, lineCount / timeDelta), Info)
			self.__flag = 1
			return False
		except BaseException as e:
...
def main() -> int:
	dynamicMergeJoin = DynamicMergeJoin()
	if not dynamicMergeJoin.initialize(													\
